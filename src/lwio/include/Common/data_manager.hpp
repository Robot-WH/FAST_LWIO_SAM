 ...
 */    
    ...
    {
        public:
            DataContainerImpl(uint16_t const& capacity) : DataContainer(capacity), type_info_(typeid(_DataT))
            {}
            // 获取数据类型  
    ...
    class DataManager
    {
        ...
        private:
            std::unordered_map<std::string, std::unique_ptr<DataContainer>> data_container_store_;  
    }; // class 
} // namespace 