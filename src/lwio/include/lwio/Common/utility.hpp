...
public:
    ...
    static _T Interpolate(const _T& front_value, const _T& back_value, const double& front_time, 
            const double& back_time, const double& time) {
        _T interpolation_v;
 ...
 */    
template<>
sensor::ImuData Utility::Interpolate<sensor::ImuData>(const sensor::ImuData& front_value, 
                                                                                                                    const sensor::ImuData& back_value, 
                                                                                                                    double const& front_time, double const& back_time, 
                                                                                                                    double const& time) {
    float front_coeff = (back_time - time) / (back_time - front_time);
    float back_coeff = (time - front_time) / (back_time - front_time);
    sensor::ImuData interpolate_data; 
...
/**
 * @brief: 线性插值的模板函数
 * @details: WheelOdom数据的特化  
 */    
template<>
sensor::WheelOdom Utility::Interpolate<sensor::WheelOdom>(const sensor::WheelOdom& front_value, 
                                                                                                                    const sensor::WheelOdom& back_value, 
                                                                                                                    double const& front_time, double const& back_time, 
                                                                                                                    double const& time) {
    float front_coeff = (back_time - time) / (back_time - front_time);
    float back_coeff = (time - front_time) / (back_time - front_time);
    sensor::WheelOdom interpolate_data; 
    interpolate_data.velocity_ = front_coeff * front_value.velocity_ + back_coeff * back_value.velocity_;  
    interpolate_data.yaw_angular_vel_ = front_coeff * front_value.yaw_angular_vel_ 
                                                                                    + back_coeff * back_value.yaw_angular_vel_;  
    return interpolate_data;  
} 
