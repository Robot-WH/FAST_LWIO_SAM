...
public:
    ...
    ALioRosNode() {
        ...
        imu_subscriber_ = node_handle_.subscribe(imu_topic, 1000, &ALioRosNode::imuHandler,
                                                    this, ros::TransportHints().tcpNoDelay());
        std::string odom_topic = RosReadParam<string>(node_handle_, "OdomTopic");
        wheel_odom_subscriber_ = node_handle_.subscribe(
            odom_topic, 2000, &ALioRosNode::OdomCallback, this, ros::TransportHints().tcpNoDelay());
        std::string gnss_topic = RosReadParam<string>(node_handle_, "GnssTopic");
        gnss_subscriber_ = node_handle_.subscribe(gnss_topic, 1000, &ALioRosNode::gnssHandler, 
                                                    this, ros::TransportHints().tcpNoDelay());  
        ...
        if (type == LidarType::Livox) {
            ...
            lidar_subscriber_ = node_handle_.subscribe(
                lidar_topic, 5, &ALioRosNode::LivoxCallBack, this, ros::TransportHints().tcpNoDelay());
            LOG(INFO) << "lidar type: Livox"; 
        } else if (type == LidarType::Velodyne || type == LidarType::Ouster) {
            lidar_subscriber_ = node_handle_.subscribe(
                lidar_topic, 5, &ALioRosNode::StandardLidarCallback, this, ros::TransportHints().tcpNoDelay());
        ...
        comm::IntraProcess::Server::Instance().Subscribe("odom_res",
            &ALioRosNode::FusionOdomResultCallback, this);  
    }
    ...
    void FusionOdomResultCallback(const lwio::ResultInfo<UsedPointT>& data) {
        ...
        if (data.pointcloud_.find("filtered_localmap") != data.pointcloud_.end()) {
            publishCloud<UsedPointT>( &local_map_filtered_publisher_,    // 发布该点云的话题 
                                data.pointcloud_.at("filtered_localmap"),   // 点云数据   
                                stamp, "odom");     
        }
    }
    ...
    void VelodyneLidarHandler(const sensor_msgs::PointCloud2ConstPtr &cloud_msg,
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.range = sqrt(range); 
            if (added_pt.z > added_pt.range) {
                std::cout << "added_pt.z: " << added_pt.z << ",added_pt.range: " << added_pt.range << std::endl;
            }
