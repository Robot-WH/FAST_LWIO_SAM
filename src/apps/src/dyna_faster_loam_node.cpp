 ...
 * @FileName: 
 * @Author: lwh
 * @Description:  dyna_faster_loam
 *                                  特点： 1、基于faster_loam
 *                                                 2、对动态环境进行处理(1、提高动态环境下匹配的鲁棒性 2、滤除动态物体的点云)
 */
...
class FusionOdometryNode {
    public:
        ...
        void Init() {
            ...
            RotaryLidarPreProcess<PointInT,PointUseT>::Option preprocess_option; 
            preprocess_option.scan_period_ = 0.0518;  
            preprocess_option.max_range_thresh_ = 100;  
            preprocess_.reset(new RotaryLidarPreProcess<PointInT,PointUseT>(preprocess_option)); 

            // PointCloudRangeImageSegmentation<PointUseT>::Option seg_option; 
            // seg_option.horizon_angle_resolution_ = 0.386;
            // seg_option.segment_angle_thresh_ = 0.05;
            // segmentation_.reset(new PointCloudRangeImageSegmentation<PointUseT>(seg_option)); 
            PointCloudSegmentation<PointUseT>::Option seg_option; 
            seg_option.horizon_angle_resolution_ = 0.386;
            seg_option.segment_angle_thresh_ = 0.05;
            segmentation_.reset(new PointCloudSegmentation<PointUseT>(seg_option)); 
            // PointCloudSegmentation<PointUseT>::Option seg_option; 
            // segmentation_.reset(new PointCloudSegmentation<PointUseT>(seg_option)); 
        }
        ...
        void publishCloud() {
            ...
            if (nonground_pointcloud_publisher_.getNumSubscribers() != 0) {
                pcl::PointCloud<PointUseT> const& nonground_points = 
            ...
            if (ground_pointcloud_publisher_.getNumSubscribers() != 0) {
                pcl::PointCloud<PointUseT> const& ground_points = 
            ...
            if (stable_pointcloud_publisher_.getNumSubscribers() != 0) {
                pcl::PointCloud<PointUseT> const& stable_points = 
            ...
            if (unstable_pointcloud_publisher_.getNumSubscribers() != 0) {
                pcl::PointCloud<PointUseT> const& unstable_points = 
            ...
            if (outlier_pointcloud_publisher_.getNumSubscribers() != 0) {
                pcl::PointCloud<PointUseT> const& outlier_points = 
        ...
        void pubMarker() {
            // if (markers_publisher_.getNumSubscribers()) {
                std::vector<PointCloudSegmentation<PointUseT>::ClusterInfo> const& cluster_info 
                    = segmentation_->GetClusterInfo();
                // std::vector<PointCloudRangeImageSegmentation<PointUseT>::ClusterInfo> const& cluster_info 
                //     = segmentation_->GetClusterInfo();
    ...
    private:
        ...
        std::deque<LidarData<PointInT>> lidar_buf_;
        /**
        ...
        std::unique_ptr<RotaryLidarPreProcess<PointInT,PointUseT>> preprocess_;  
        //std::unique_ptr<PointCloudRangeImageSegmentation<PointUseT>> segmentation_;  
        std::unique_ptr<PointCloudSegmentation<PointUseT>> segmentation_;  
