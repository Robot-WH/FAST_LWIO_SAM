...
/**
 * @brief IMU预积分残差因子  
 *  15： 状态总维数    p,v,q,ba,bg
 *  7:关联的node的状态 维数 pi + qi：7
 * 9: 关联的node的状态 维数 vi + bai + bgi：9
  *  7:关联的node的状态 维数 p(i+1) + q(i+1)：7
 * 9: 关联的node的状态 维数 v(i+1) + ba(i+1) + bg(i+1)：9
 */
class IMUPreIntegrationFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9> {
  public:
    IMUPreIntegrationFactor() = delete;
    IMUPreIntegrationFactor(ImuPreIntegrationBase* pre_integrations):pre_integration_(pre_integrations)
    {}
...
//delta_q = Qi.inverse() * Qj;
...
#if 0
        if ((Bai - pre_integration_->linearized_ba).norm() > 0.10 ||
            (Bgi - pre_integration_->linearized_bg).norm() > 0.01)
        {
        ...
        if (jacobians) {
            ...
            if (pre_integration_->jacobian.maxCoeff() > 1e8 || pre_integration_->jacobian.minCoeff() < -1e8) {
                //std::cout << pre_integration_->jacobian << std::endl;
///                ROS_BREAK();
            }
            ...
            if (jacobians[0]) {
                ...
                jacobian_pose_i.block<3, 3>(O_P, O_R) = 
                    Math::GetSkewMatrix(Qi.inverse() * (0.5 * G * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));

#if 0
            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
#else
                Eigen::Quaterniond corrected_delta_q = pre_integration_->delta_q_ * Math::deltaQ(dq_dbg * (Bgi - pre_integration_->linearized_bg_));
                jacobian_pose_i.block<3, 3>(O_R, O_R) = 
                    -(Math::QLeft(Qj.inverse() * Qi) 
                        * Math::QRight(corrected_delta_q)).bottomRightCorner<3, 3>();
#endif

                jacobian_pose_i.block<3, 3>(O_V, O_R) = Math::GetSkewMatrix(Qi.inverse() * (G * sum_dt + Vj - Vi));
            ...
            jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -dq_dbg;
                ...
                jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = 
                    -Math::QLeft(Qj.inverse() * Qi * pre_integration_->delta_q_).bottomRightCorner<3, 3>() * dq_dbg;
#endif
...
#else
                Eigen::Quaterniond corrected_delta_q = pre_integration_->delta_q_ * Math::deltaQ(dq_dbg * (Bgi - pre_integration_->linearized_bg_));
                jacobian_pose_j.block<3, 3>(O_R, O_R) = Math::QLeft(corrected_delta_q.inverse() * Qi.inverse() * Qj).bottomRightCorner<3, 3>();
#endif
    ...
    ImuPreIntegrationBase* pre_integration_;
};
}
}
