...
public:  
    ...
    bool CalibExTranslationPlanar() {
        ext_q_result_.setIdentity();
        Eigen::MatrixXd A = Eigen::MatrixXd::Zero(pose_storage_.size() * 2, 4);
        Eigen::MatrixXd b = Eigen::MatrixXd::Zero(pose_storage_.size() * 2, 1);
        ...
        for (size_t i = 0; i < pose_storage_.size(); i++) {
            ...
            // double huber = t_dis > 0.04 ? 0.04 / t_dis : 1.0;
            // 校正primary传感器的姿态
            Eigen::Quaterniond primary_q = pose_primary.q();    // 主传感器的旋转 
            // std::cout << "sub_q: " << sub_q.w() << ", " << sub_q.vec().transpose() << std::endl;
            // std::cout << "primary_q: " << primary_q.w() << ", " << primary_q.vec().transpose() << std::endl;
            // 将主传感器的旋转调整为与z轴平行
            primary_q.x() = 0; 
            ...
            Eigen::Vector3d sub_t_in_planar = ext_q_result_ * pose_sub.t();
            // Eigen::Vector3d sub_t_in_planar = pose_sub.t();
            // std::cout << "sub_t_in_planar: " << sub_t_in_planar.transpose() << std::endl;
            // std::cout << "pose_primary t: " << pose_primary.t().transpose() << std::endl;
            A.block<2, 2>(i * 2, 0) = (primary_q.toRotationMatrix() - Eigen::Matrix3d::Identity()).topLeftCorner<2, 2>();
            A(i * 2, 2) = -sub_t_in_planar.x();
            A(i * 2, 3) = sub_t_in_planar.y(); 
            A(i * 2 + 1, 2) = -sub_t_in_planar.y(); 
            A(i * 2 + 1, 3) = -sub_t_in_planar.x(); 
            b.block<2, 1>(i * 2, 0) = -pose_primary.t().head(2);
        }
        Eigen::Vector4d x; 
        /**
         *  TODO: 看看和QR分解的差别 
        ...
        x = A.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(b);
        // ext_t_result_ = x;
        double alpha = std::atan(x[3] / x[2]);
        Eigen::Quaterniond q_z(std::cos(alpha / 2), 0, 0, std::sin(alpha / 2));
        ext_q_result_ = q_z * ext_q_result_;    // q_z * q_yz
        std::cout << "外参平移 x: " << x[0] << ",y: " << x[1] << std::endl;
        std::cout << "外参旋转：" << std::endl <<
            ext_q_result_.toRotationMatrix() << std::endl;
        return true;  
    }
...
protected:
    ...
    bool checkScrewMotion(const Pose &pose_primary, const Pose &pose_sub, const bool& trans_valid) {
        ...
        if (ang_axis_accum_pri.angle() > 0.0 || ang_axis_accum_sub.angle() > 0.0) {
            return true;  
        }
        return false;  
    }
